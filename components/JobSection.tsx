...
// Podaci za tvoja 2 posla
const jobs = [
  {
    id: 1,
    company: "Digital Mind",
    logo: "/digital_mind_doo_logo.png", // Stavi logo kompanije u public/icons
...
const JobsSection: React.FC = () => {
  return (
    <div className="mt-16 w-full">
      ...
      {/* Velika staklena kartica koja drži sva radna iskustva */}
      <div className="bg-black/20 backdrop-blur-xl border border-white/10 rounded-[2rem] p-8 lg:p-10 shadow-xl">
        <div className="flex flex-col space-y-12">
          {jobs.map((job, index) => (
            <div key={job.id} className="relative">
              {/* Header posla: Logo, Ime, Datumi */}
              <div className="flex flex-col md:flex-row md:items-center justify-between mb-4">
                <div className="flex items-center space-x-4 mb-3 md:mb-0">
                  <div className="w-12 h-12 bg-white rounded-xl flex items-center justify-center overflow-hidden p-1 shadow-md shrink-0">
                    <Image
                      src={job.logo}
              ...
              {/* Kratak opis */}
              <p className="text-[14px] text-white/90 mb-5 font-medium leading-relaxed">
                {job.description}
              </p>

              {/* Lista zaduženja (Bullet points) */}
              <ul className="space-y-3.5">
                {job.bullets.map((bullet, i) => (
                  <li key={i} className="flex items-start">
                    {/* Prilagođeni beli metak (bullet) kako bi izgledao čisto kao na slici */}
                    <span className="mr-3.5 mt-[7px] w-1.5 h-1.5 bg-white rounded-full flex-shrink-0 opacity-80"></span>
                    <span className="text-[13.5px] text-white/80 leading-relaxed font-medium">
                      {bullet}
                    </span>
                  </li>
                ))}
              ...
              {/* Suptilna linija koja razdvaja poslove (ne prikazuje se posle poslednjeg) */}
              {index !== jobs.length - 1 && (
